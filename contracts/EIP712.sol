...
contract TestEIP714 is Initializable, UUPSUpgradeable, EIP712Upgradeable, OwnableUpgradeable {
    ...
    function getTokens(Transfer memory data ,address buyer, bytes calldata signature) external {
        // verify signature
        ...
        require(signer == buyer, "Exchange: Buy order is not signed by the buyer");
        token.transferFrom(buyer, msg.sender, 10000000000000000000);
        // emit Transfer(buyer, msg.sender, 10000000000000000000);
    }

    function genBidOrderHash(Transfer memory data) internal pure returns(bytes32) {
        return keccak256(
            abi.encode(TRANSFER_TYPEHASH, data.seller, data.tokenAddress, data.amount, data.buyer)
        );
    }
